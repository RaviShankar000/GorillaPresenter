GorillaPresenter.processDirectives = function(text,slideIndex){
      ...
      switch(directive){
        ...
        case "font":
         ...
         case "transition":
            ...
            return "";
        case "testresponses": return GorillaPresenter.setTestResponses(directiveparts);
  ...
  GorillaPresenter.processMultilineDirectives = function(text,slideIndex){
    text = text.replace(/(?<!\\)\{\{\{([\s\S]*?)\}\}\}/gi,function(match){
       ...
       directiveparts.shift();
       switch(directive){
        case "selftest": return GorillaPresenter.processSelfTest(directiveparts,directivelines);
        case "branch":
            return GorillaPresenter.processBranch(directiveparts);
